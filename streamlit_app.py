...
with tab1:
    ...
    st.subheader("Daily PM10 Trends (Districts)")
    daily_pm10 = pol_filt.groupby(['Date','자치구'])['미세먼지(PM10)'].mean().unstack()
    st.line_chart(daily_pm10)

    st.subheader("Average PM10 Concentration by District")
    avg_pm10 = pol_filt.groupby('자치구')['미세먼지(PM10)'].mean().reindex(gus_tab1)
    fig, ax = plt.subplots()
    ...
    st.subheader("District-wise PM10 Map")
...
with tab2:
    ...
    fig, ax = plt.subplots()
    pt_usage.plot(kind='bar', ax=ax, color='lightcoral')
    ax.set_xlabel("District")
    ax.set_ylabel("Number of Passengers")
    st.pyplot(fig)

    st.subheader("Floating Population (2012 vs 2014)")
...
with tab3:
    ...
    if '전체' in delivery.columns:
        st.line_chart(delivery_filt.set_index('Date')['전체'])
    else:
