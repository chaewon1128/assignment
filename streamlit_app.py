...
def load_data():
    ...
    if delivery is None or delivery.empty:
        ...
        delivery = delivery.rename(columns={'전체': 'Delivery Index'})
...
with tab1:
    ...
    if pol_filt.empty:
        ...
        ax.set_xlabel("District", fontsize=12)  # X축 변경
        ax.set_ylabel("Avg PM10 (μg/m³)", fontsize=12)  # Y축 변경
        ax.set_title(f"Avg PM10 by District in Selected Years ({', '.join(selected_years)})", fontsize=14)  # 타이틀 변경
    ...
    if mobility_filt.empty:
        ...
        with col1:
            ...
            if not daily_comp_mobility.empty:
                ...
                ax2 = ax1.twinx()
                ax1.plot(daily_comp_mobility['Date'], daily_comp_mobility['미세먼지(PM10)'], color='blue', label='PM10 Concentration')
                ax1.set_xlabel("Date")  # X축 영어
                ax1.set_ylabel("PM10 (μg/m³)", color='blue')
                ax1.tick_params(axis='y', labelcolor='blue')
                ax2.plot(daily_comp_mobility['Date'], daily_comp_mobility['승객_수'], color='green', label='Total Transit Volume')
                ax2.set_ylabel("Total Transit Volume", color='green')
                ax2.tick_params(axis='y', labelcolor='green')
                ax1.set_title("Daily PM10 Concentration and Transit Volume", fontsize=14)  # 제목 영어
                fig.tight_layout()
                st.pyplot(fig)
            else:
        ...
        with col2:
            ...
            if not avg_transit_by_pm10.empty:
                ...
                ax.set_xlabel("PM10 Status", fontsize=12)  # X축 영어
                ax.set_ylabel("Avg Transit Volume", fontsize=12)  # Y축 영어
                ax.set_title("Avg Transit Volume by PM10 Status")  # 제목 영어
...
with tab3:
    ...
    if not delivery_comp_filt.empty:
        fig, ax1 = plt.subplots(figsize=(10, 5))
        ax2 = ax1.twinx()
        ax1.plot(delivery_comp_filt.index, delivery_comp_filt['미세먼지(PM10)'], color='orange', label='PM10 Concentration')
        ax1.set_ylabel("PM10 (μg/m³)", color='orange')
        ax1.tick_params(axis='y', labelcolor='orange')
        ax2.plot(delivery_comp_filt.index, delivery_comp_filt['Delivery Index'], color='red', label='Delivery Index')
        ax2.set_ylabel("Delivery Index", color='red')
        ax2.tick_params(axis='y', labelcolor='red')
        ax1.set_title(f"Change in PM10 and Delivery Index in {year_select_tab3}", fontsize=14)
        fig.tight_layout()
        st.pyplot(fig)
        st.caption("Higher PM10 concentration tends to associate with increased delivery demand.")
    else:
    ...
    if not map_data_tab3.empty and map_data_tab3['Avg_Spending'].max() > 0:
        ...
        st.caption("Circle size shows avg spending (proxy for delivery demand), color shows PM10 status.")
    else:
...
with tab4:
    ...
    corr_df_gu = pd.DataFrame({
        "PM10": pm10_avg_gu,
        "Transit Volume": transit_avg_gu,        # 영어 컬럼명
        "Avg Spending": spending_avg_gu          # 영어 컬럼명
    }).dropna()
    if not corr_df_gu.empty and len(corr_df_gu) >= 2:
        corr_mat = corr_df_gu.corr(method='pearson')
        fig, ax = plt.subplots(figsize=(10, 10)) # 그래프 확대
        sns.heatmap(
            corr_mat,
            annot=True,
            cmap='vlag',
        ...
        ax.set_title("Correlation between PM10, Transit Volume, Avg Spending", fontsize=16)  # 타이틀 영어
    ...
    if not combined_ppl.empty and not pol_filt.empty:
        ...
        if not ppl_pm10_comp.empty and len(ppl_pm10_comp) >= 2:
            ...
            ax.axvline(ppl_pm10_comp['평균_PM10'].mean(), color='r', linestyle='--', linewidth=1, label='Mean PM10')
            ax.axhline(0, color='k', linestyle='-', linewidth=1, label='Population Change 0')
            ax.set_title("Change in Population Flow and PM10 (2014 - 2012)", fontsize=14)  # 타이틀 영어
            ax.set_xlabel("Avg PM10 Concentration (Selected Year)", fontsize=12)  # X축 영어
            ax.set_ylabel("Change in Population Flow (2014 - 2012)", fontsize=12)  # Y축 영어
